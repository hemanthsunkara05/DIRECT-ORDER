...
**Status:** Phase 10 (Restaurant order management) — see [PRODUCT/docs/13-implementation-phases.md](PRODUCT/docs/13-implementation-phases.md). Foundation (Phase 1), core schema and tenancy (Phase 2), authentication (Phase 3), authorization (Phase 4), restaurant creation/onboarding/profile/staff/uploads (Phase 5), menu categories/items (Phase 6), the public ordering page (Phase 7), the pricing engine + server-side cart validation (Phase 8), order creation/checkout/payment verification/webhooks/refunds (Phase 9), and the restaurant order queue with accept/reject/preparing/ready and a live SSE feed (Phase 10) are done. A customer can persist a cart, check out, pay (a mock provider by default — see Payments below), and track their order by guest access token; a restaurant can watch orders arrive live, accept or reject (with an automatic full refund) them, and move them through preparation to ready-for-pickup — every write server-verified, idempotent, and tenant-isolated. As of Phase 10 the full flow — registration through order fulfillment — has been run end-to-end against a real PostgreSQL/Redis stack via `docker compose up -d`, not just the in-memory test fakes; see Testing below.
...
- **Unit / integration:** Vitest, per workspace (`apps/api/test`, `packages/money/test`, ...). API integration tests boot a real NestJS + Fastify application over real HTTP (via supertest); `PrismaService` and `RedisService` are overridden with in-memory stand-ins (`apps/api/test/support/`) rather than requiring a live database and Redis in every environment that runs the suite — see the note at the top of `apps/api/test/health.e2e.test.ts` and `apps/api/test/support/create-test-app.ts`. 367 tests across `apps/api` as of Phase 10 (431 repo-wide), including all ten named failure scenarios from `docs/11-testing-strategy.md` §18.3 (double-click pay, browser crash after payment, duplicate webhook, refresh after payment, price change/item unavailable mid-checkout, provider/backend amount mismatch, invalid webhook signature, cross-customer order access, concurrent identical checkouts), the full order state graph, the refund concurrency guard, and Phase 10's restaurant-order tenant isolation / concurrent-accept / double-rejection-refund / SSE-replay coverage.
...
- **Full-stack / live database:** CI runs a dedicated job (`live-database-smoke-test`) that starts a real PostgreSQL service and the actual compiled API against it, then polls `/health` and `/ready` — this is the closest equivalent to `docker compose up -d && pnpm db:migrate && pnpm dev` that runs automatically on every change. See `.github/workflows/ci.yml`. As of Phase 10, this has also been run manually in the development sandbox itself (`docker compose up -d && pnpm db:migrate && pnpm db:grants && pnpm db:seed`), with a full curl-driven walkthrough of registration through order fulfillment against the real containers — see the Restaurant order management section above and PHASE_REPORTS.md for what that verified.
