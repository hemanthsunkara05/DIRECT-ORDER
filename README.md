...
**Status:** Phase 12 (Notifications) — see [PRODUCT/docs/13-implementation-phases.md](PRODUCT/docs/13-implementation-phases.md). Foundation (Phase 1), core schema and tenancy (Phase 2), authentication (Phase 3), authorization (Phase 4), restaurant creation/onboarding/profile/staff/uploads (Phase 5), menu categories/items (Phase 6), the public ordering page (Phase 7), the pricing engine + server-side cart validation (Phase 8), order creation/checkout/payment verification/webhooks/refunds (Phase 9), the restaurant order queue with accept/reject/preparing/ready and a live SSE feed (Phase 10), delivery dispatch + provider webhooks (Phase 11), and the notification pipeline + centre + preferences (Phase 12) are done. A customer can persist a cart, check out, pay (a mock provider by default — see Payments below), and track their order — including live courier status — by guest access token; a restaurant can watch orders arrive live, accept or reject (with an automatic full refund) them, move them through preparation to ready-for-pickup, have a courier dispatched automatically the moment it's marked ready, and receive an in-app/SMS/WhatsApp notification at every step. **Phase 11 was the end of the pilot scope** (docs/13-implementation-phases.md: "At this point the product is commercially usable... deploy to staging and run a real pilot before continuing"); Phase 12 is the first phase explicitly built past that milestone, per the user's standing instruction to continue through all 20 phases without stopping for go-ahead. As of Phase 12 the full flow — registration through order fulfillment through delivery dispatch, a provider webhook, and a real notification landing in a real database — has been run end-to-end against a real PostgreSQL/Redis stack via `docker compose up -d`, not just the in-memory test fakes; see Testing below.
...
- **Full-stack / live database:** CI runs a dedicated job (`live-database-smoke-test`) that starts a real PostgreSQL service and the actual compiled API against it, then polls `/health` and `/ready` — this is the closest equivalent to `docker compose up -d && pnpm db:migrate && pnpm dev` that runs automatically on every change. See `.github/workflows/ci.yml`. As of Phase 12, this has also been run manually in the development sandbox itself (`docker compose up -d && pnpm db:migrate && pnpm db:grants && pnpm db:seed`), with a full curl-driven walkthrough of registration through order fulfillment through delivery dispatch, a provider webhook, and a real notification landing in a real database, against the real containers and the real outbox relay timer (not a test harness) — see the Restaurant order management, Delivery, and Notifications sections above and PHASE_REPORTS.md for what that verified.
