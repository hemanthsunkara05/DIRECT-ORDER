...
| POST         | `/restaurant/orders/:id/cancel`           | MANAGER   | `orders:cancel` (BR-174) — cancellation-after-accept, reason required, full refund                                                                                            |
...
| GET          | `/admin/overview/command-center`                               | `analytics:platform` | Phase 23a — KPIs (rollup-sourced), order-lifecycle funnel (live), priority alerts (stuck orders, overdue approvals — both live, threshold constants documented on the controller), top restaurants by order count over the last 7 days |
...
| POST         | `/admin/restaurants/:restaurantId/uploads/presign`              | `restaurant:admin_edit` | Phase 23a (TODOS.md) — admin-authorized equivalent of `/restaurant/uploads/presign`, gated by role not tenant membership. Reuses `UploadService` unchanged                     |
| POST         | `/admin/restaurants/:restaurantId/uploads/verify`               | `restaurant:admin_edit` | Phase 23a — same as above; audit log records `actorType: 'ADMIN'`                                                                                                               |
...
| GET          | `/admin/orders/directory`                                       | Any admin   | City -> restaurant order-count tree for the Orders & payments navigation view                                                                                                  |
| GET          | `/admin/orders/:id/detail`                                      | Any admin   | Phase 23a — full cross-tenant order detail, including delivery status/courier info (work item 3)                                                                               |
