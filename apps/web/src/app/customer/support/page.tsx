...
function SupportCasesView() {
  ...
  const load = useCallback(async () => {
    try {
      const page = await supportApi.listMine();
      setCases(page.items);
      setError(null);
    } catch (err) {
    ...
    <main className="mx-auto flex min-h-screen max-w-lg flex-col gap-6 p-8">
      ...
      {cases === null ? (
        ...
        <ul className="flex flex-col divide-y divide-slate-200 rounded-lg border border-slate-200">
          {cases.map((c) => (
            <li key={c.id}>
              <Link href={`/customer/support/${c.id}`} className="flex flex-col gap-1 p-3 text-sm hover:bg-slate-50">
                <div className="flex items-center justify-between">
...
function NewCaseForm({ onCreated }: { onCreated: () => Promise<void> }) {
