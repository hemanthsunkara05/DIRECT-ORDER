...
export default function RestaurantSupportCaseDetailPage() {
  return (
    <ProtectedRoute>
      <CaseDetail />
    </ProtectedRoute>
...
function CaseDetail() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setDetail(await supportApi.getRestaurant(params.id, restaurantId));
      setError(null);
    } catch (err) {
  ...
  async function handleReply(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const attachments = file ? [await uploadSupportAttachment(params.id, file, 'restaurant')] : undefined;
      await supportApi.replyRestaurant(params.id, body.trim(), attachments, restaurantId);
      setBody('');
      setFile(null);
      await load();
    } catch (err) {
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-lg flex-col gap-6 p-8">
      <Link href="/restaurant/support" className="text-xs text-slate-400 hover:underline">
        ← Back to support
      </Link>
      <div>
        ...
        {detail.messages.map((m) => (
          <div key={m.id} className="rounded-lg border border-slate-200 p-3 text-sm">
            <p className="text-xs font-medium text-slate-500">
              {m.authorType === 'AGENT' ? 'Support' : m.authorType === 'RESTAURANT' ? 'You' : m.authorType}
            </p>
